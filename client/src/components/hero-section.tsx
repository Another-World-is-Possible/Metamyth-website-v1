      ...
      <section className="min-h-screen flex items-center justify-center relative overflow-hidden pt-16">
        ...
        <div 
          className={`absolute inset-0 bg-cover bg-center bg-no-repeat transition-opacity duration-2000 ease-out ${
            backgroundLoaded && showContent ? 'opacity-100' : 'opacity-0'
          }`}
          style={{ backgroundImage: backgroundLoaded ? `url(${heroBackground})` : 'none' }}
        />
        {/* Dark overlay for text readability */}
        <div className={`absolute inset-0 bg-deep-black/40 z-10 transition-opacity duration-1000 ${
        ...
        <div className="relative z-10 text-center max-w-6xl mx-auto px-4" style={{ transform: 'translateY(-25px)' }}>
          <motion.h1 
            ...
            animate={{ opacity: backgroundLoaded && showContent ? 1 : 0, y: backgroundLoaded && showContent ? 0 : 20 }}
            transition={{ duration: 0.8, ease: "easeOut", delay: backgroundLoaded && showContent ? 1.5 : 0 }}
          >
          ...
          <motion.h2
            ...
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: backgroundLoaded && showContent ? 1 : 0, y: backgroundLoaded && showContent ? 0 : 20 }}
            transition={{ delay: backgroundLoaded && showContent ? 1.8 : 0, duration: 0.8, ease: "easeOut" }}
          ...
          <motion.p
            ...
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: backgroundLoaded && showContent ? 1 : 0, y: backgroundLoaded && showContent ? 0 : 20 }}
            transition={{ delay: backgroundLoaded && showContent ? 2.1 : 0, duration: 0.8, ease: "easeOut" }}
          ...
          <motion.div
            ...
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: backgroundLoaded && showContent ? 1 : 0, y: backgroundLoaded && showContent ? 0 : 20 }}
            transition={{ delay: backgroundLoaded && showContent ? 2.4 : 0, duration: 0.8, ease: "easeOut" }}
